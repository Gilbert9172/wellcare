...
NON_DATETIME_SELECT = 'SELECT time FROM resv_non_datetime WHERE date=:resv_date'
...
class Check(Resource):
    ...
    def post(self):
        ...
        # 예약 날짜시간 막기
        row2 = app.db.execute(text(NON_DATETIME_SELECT), q).fetchall()
        
        if row2 != None:
            for index in range(len(time_list)):
                for idx in range(len(row2)):
                    if row2[idx]['time'] == time_list[index]['time']:
                        time_list[index]['isPossible'] = False
